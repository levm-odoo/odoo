...
class Field(typing.Generic[T], metaclass=MetaField):
    ...
    default: Callable[[BaseModel], T] | None = None  # default(recs) returns the default value
    ...
    group_expand: str | Callable[[BaseModel, ModelType, DomainType], ModelType] | None = None  # name of method to expand groups in read_group()
    ...
    def setup_related(self, model: BaseModel) -> None:
        ...
        assert isinstance(self.related, str) and self.related, f"Missing {self}.related"
    ...
    def _search_related(self, records: BaseModel, operator: str, value) -> DomainType:
        ...
        def make_domain(path, model: BaseModel) -> DomainType:
            if '.' not in path:
            ...
            domain: DomainType = [(prefix, 'in', comodel._search(make_domain(suffix, comodel)))]
