    ...
    def _process_jobs(db_name, soft_limit=None):
        ...
        try:
            ...
            with db.cursor() as cron_cr:
                ...
                job_queue = [job['id'] for job in reversed(jobs)]
                del jobs  # not needed anymore
                ...
                while True:
                    if not job_queue:
                        ...
                        job_queue.extend(job['id'] for job in reversed(cls._get_all_ready_jobs(cron_cr)))
                        if job_queue or all(job_id in locked_jobs for job_id in job_queue):
                            break
                        locked_jobs.clear()
