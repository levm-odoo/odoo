...
class TestWebReadGroup(common.TransactionCase):
    ...
    def test_count_read_groups(self):
        ...
        self.assertEqual(
            ...
            {'groups': [{'__extra_domain': [(1, '=', 1)], 'key:count': 2}], 'length': 1},
        )

        self.assertEqual(
            Model.web_read_group([], aggregates=['key:count_distinct']),
            {'groups': [{'__extra_domain': [(1, '=', 1)], 'key:count_distinct': 1}], 'length': 1},
        )
        ...
        for timezone_key, excepted in excepted_results.items():
            with self.subTest(f"groupby absolute week with {timezone_key=}"):
                ...
                self.assertDictEqual(
                    {week['datetime:week'][1]: week['__count'] for week in groups['groups']},
                    {week['date:week'][1]: week['__count'] for week in excepted['groups']},
    ...
    def test_groupby_many2many(self):
        ...
        for group in result['groups']:
            self.assertEqual(
                ...
                'A search using the domain returned by the web_read_group should give the '
    ...
    def test_related(self):
        ...
        self.assertEqual(
            ...
            {'groups': [{'__extra_domain': [(1, '=', 1)], 'foo_id_bar_id_name:count_distinct': 1}], 'length': 1},
        )
    ...
    def test_performance_prefetch_fold_display_name(self):
        ...
        Line = self.env['test_read_group.order.line'].with_context(read_group_expand=True)
        ...
        with self.assertQueryCount(2):  # 1 for web_read_group + 1 for fetch display_name/fold
            ...
            self.assertEqual(
                ...
                {
                    'groups': [
                        {'order_id': (order_1.id, 'O1'), '__extra_domain': [('order_id', '=', order_1.id)], 'value:sum': 1},
                        {'order_id': (order_2.id, 'O2'), '__extra_domain': [('order_id', '=', order_2.id)], 'value:sum': 4},
                        {'order_id': (order_3.id, 'O3'), '__extra_domain': [('order_id', '=', order_3.id)], 'value:sum': 3},
                        {'order_id': False, '__extra_domain': [('order_id', '=', False)], 'value:sum': 3},
                    ],
                    'length': 4,
                },
            )
        ...
        with patch.object(self.registry['test_read_group.order'], '_compute_display_name', side_effect=old_compute_display_name, autospec=True) as compute_display_name_spy:
            self.assertEqual(
                Line.web_read_group([], ['order_id'], ['value:sum']),
                {
                    'groups': [
                        {
                            '__extra_domain': [('order_id', '=', order_1.id)],
    ...
    def test_order_by_many2one_id_perf(self):
        ...
        with self.assertQueries([expected_query + ' ASC']):
            OrderLine.web_read_group([], ["order_id"], ["__count"])
        with self.assertQueries([expected_query + ' DESC']):
            OrderLine.web_read_group([], ["order_id"], ["__count"], order="order_id DESC")
