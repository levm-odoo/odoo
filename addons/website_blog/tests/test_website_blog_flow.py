...
class TestWebsiteBlogTranslationFlow(HttpCase, TestWebsiteBlogCommon):
    ...
    def test_update_field_translation(self):
        """Checks if translated text could be updated"""
        ...
        blog_post_english = blog_post.with_context(lang=en_lang.code)
        blog_post_portugues = blog_post.with_context(lang=br_lang.code)

        # Ensure that initial translations for 'en_US' and 'pt_BR' are different
        blog_post.update_field_translations('content', {en_lang.code: {'Todos os blogs' : 'All blogs'}})
        self.assertEqual('Todos os blogs', blog_post_portugues.content)
        self.assertEqual('All blogs', blog_post_english.content)
