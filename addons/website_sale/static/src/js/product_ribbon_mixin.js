...
export function productRibbonMixin(component) {
  return component.extend({
    ...
    async setRibbon(previewMode, widgetValue, params) {
        if (previewMode === 'reset') {
            ...
            this.prevRibbon = {
                ...
                'position': this.ribbon.classList.contains('o_ribbon_left')
                    || this.ribbon.classList.contains('o_tag_left') ? 'left' : 'right',
                'style': this.ribbon.classList.contains('o_ribbon_left')
                    || this.ribbon.classList.contains('o_ribbon_right') ? 'ribbon' : 'tag',
    ...
    async setRibbonPosition(previewMode, widgetValue, params) {
        const currentStyle = (
            this.ribbon.classList.contains('o_ribbon_left')
            || this.ribbon.classList.contains('o_ribbon_right')
        )? 'ribbon' : 'tag';
    ...
    async setRibbonStyle(previewMode, widgetValue, params) {
        const currentPosition = (
            this.ribbon.classList.contains('o_ribbon_left')
            || this.ribbon.classList.contains('o_tag_left')
        )? 'left' : 'right';
        this.ribbon.className = this.ribbon.className.replace(
    ...
    async _saveRibbon(isNewRibbon = false) {
        ...
        const ribbon = {
            ...
            'text_color': this.ribbon.style.color,
            'position': (
                this.ribbon.className.includes('o_ribbon_left')
                || this.ribbon.className.includes('o_tag_left')
            ) ? 'left' : 'right',
            'style': (
                this.ribbon.className.includes('o_ribbon_left')
                || this.ribbon.className.includes('o_ribbon_right')
            ) ? 'ribbon': 'tag',
        };
    ...
    async _setRibbon(ribbonId, del = false) {
        ...
        ribbons.forEach(ribbonElement => {
            ...
            if (ribbon.style && ribbon.position) {
                const positionClass = this.PositionClasses[ribbon.style][ribbon.position];
                ribbonElement.classList.add(positionClass);
            }
