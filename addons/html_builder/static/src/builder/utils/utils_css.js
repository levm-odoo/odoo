...
const COLOR_PALETTE_COMPATIBILITY_COLOR_NAMES = [
    "primary",
    "secondary",
    "alpha",
    "beta",
 ...
 * @returns {number}
 */
function _convertValueToUnit(value, unitTo, cssProp) {
    ...
    const valueUnit = m[1];
    return _convertNumericToUnit(numValue, valueUnit, unitTo, cssProp);
}
/**
 ...
 * @returns {number}
 */
function _convertNumericToUnit(value, unitFrom, unitTo, cssProp) {
 ...
 * @returns {boolean}
 */
function _areCssValuesEqual(value1, value2, cssProp) {
    ...
    if (cssProp && cssProp.endsWith("-size")) {
        ...
        for (const index of [0, 1]) {
            ...
            if (!_areCssValuesEqual(part1, part2, pseudoPartProp)) {
                return false;
            }
        }
        return true;
    ...
    const numValue2 = _convertValueToUnit(value2, data[1], cssProp);
...
function _getBgImageURL(el) {
    const parts = _backgroundImageCssToParts(window.getComputedStyle(el).backgroundImage);
