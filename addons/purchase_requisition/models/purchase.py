            ...
            for line in pol.order_id.requisition_id.line_ids:
                if line.product_id == pol.product_id:
                    ...
                    if not pol.date_planned:
                        date_planned = datetime.strptime(pol.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S") if pol.date_planned_pol_placeholder else pol._get_date_planned(seller)
                        pol.date_planned = date_planned.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
