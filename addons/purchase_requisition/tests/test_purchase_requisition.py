...
class TestPurchaseRequisition(TestPurchaseRequisitionCommon):
    ...
    def test_07_alternative_purchases_wizards(self):
        ...
        alt_po_1.order_line[0].date_planned = (
            datetime.strptime(alt_po_1.order_line[0].date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S")
            + timedelta(days=1)
    ...
    def test_alternative_purchase_order_merge(self):
        ...
        merger_alternative_orders[0].order_line.date_planned = datetime.strptime(merger_alternative_orders[0].order_line.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S").strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        merger_alternative_orders[1].order_line.date_planned = datetime.strptime(merger_alternative_orders[1].order_line.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S").strftime(DEFAULT_SERVER_DATETIME_FORMAT)
