...
function correctBorderAttributes(style) {
    ...
    dummy.style = style;
    const stylesObject = dummy.getAttribute('style')
        .replace(/\s+/g, "")
        .split(";")
    ...
    if (stylesObject['border-width']) {
        BORDER_WIDTHS_ATTRIBUTES.forEach((attribute) => {
            stylesObject[attribute] = stylesObject['border-width'];
        });
    }
    ...
    // Reconstruct the style string from the updated object
