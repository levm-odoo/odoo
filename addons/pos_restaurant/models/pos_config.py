...
class PosConfig(models.Model):
    ...
    def _load_bar_demo_data(self):
        ...
        if not self.env.ref('pos_restaurant.product_category_drinks', raise_if_not_found=False):
            convert.convert_file(self.env, 'pos_restaurant', 'data/scenarios/bar_demo_data.xml', None, noupdate=True, mode='init', kind='data')
    ...
    def load_onboarding_restaurant_scenario(self, with_demo_data=True):
        if not self.env.ref('pos_restaurant.pos_resource_preset', raise_if_not_found=False):
            convert.convert_file(self.env, 'pos_restaurant', 'data/scenarios/restaurant_preset.xml', None, noupdate=True, mode='init', kind='data')
        ...
        if not self.env.ref('pos_restaurant.floor_main', raise_if_not_found=False):
            convert.convert_file(self.env, 'pos_restaurant', 'data/scenarios/restaurant_floor.xml', None, noupdate=True, mode='init', kind='data')
        if with_demo_data:
            ...
            if self.env.company.id == self.env.ref('base.main_company').id:
                ...
                if not existing_session:
                    convert.convert_file(self.env, 'pos_restaurant', 'data/scenarios/restaurant_demo_session.xml', None, noupdate=True, mode='init', kind='data')
        return {'config_id': config.id}
    ...
    def _load_restaurant_demo_data(self):
        ...
        if not self.env.ref('pos_restaurant.food', raise_if_not_found=False):
            convert.convert_file(self.env, 'pos_restaurant', 'data/scenarios/restaurant_demo_data.xml', None, noupdate=True, mode='init', kind='data')
    ...
    def _load_default_demo_data(self):
        if self.module_pos_restaurant:
            return self._load_restaurant_demo_data()
        return super()._load_default_demo_data()
