...
class CustomerPortal(Controller):
    ...
    def account(self, redirect=None, **post):
        ...
        if post and request.httprequest.method == 'POST':
            if not partner_sudo.can_edit_vat():
                post['country_id'] = str(partner_sudo.country_id.id)
            ...
            if not error:
                ...
                self.on_account_update(values, partner_sudo)
                partner_sudo.write(values)
                if redirect:
                    return request.redirect(redirect)
                return request.redirect('/my/home')
        address_form_values = self._prepare_address_form_values(partner_sudo, **post)

        response = request.render("portal.address", address_form_values)
    ...
    def _prepare_address_form_values(
        self, partner_sudo, address_type='', country_id='', **kwargs
    ):
        ...
        can_edit_vat = (
            ...
            and (partner_sudo._is_anonymous_customer() or partner_sudo.can_edit_vat())
        )

        country_sudo = (
            (
                country_id
                and self.env['res.country'].browse(int(country_id)).exists()
            ) or partner_sudo._get_country_id()
        )
        ...
        return {
            ...
            'show_vat': (
                (self._is_billing_address(address_type, **kwargs))
                and (partner_sudo._is_anonymous_customer() or (can_edit_vat or partner_sudo.vat))
            ),
            'vat_label': _("VAT"),
        }
    ...
    @route(['/portal/country_info/<model("res.country"):country>'], type='jsonrpc', auth="public", methods=['POST'], website=True)
    ...
    def _validate_address_values(self, address_values, partner_sudo, address_type='other', **_kwargs):
        ...
        required_field_set = self._get_required_address_field_set(country, address_type, **_kwargs)
    ...
    def _get_required_address_field_set(self, country, address_type='invoice', **_kwargs):
        required_field_set = set()
        check_address_type = self._check_address_type(**_kwargs)
        if not check_address_type or address_type == 'delivery':
            required_field_set |= self._get_mandatory_delivery_address_fields(country)
        if not check_address_type or address_type == 'invoice':
            required_field_set |= self._get_mandatory_billing_address_fields(country)

            return required_field_set

    def _check_address_type(**_kwargs):
        return True
