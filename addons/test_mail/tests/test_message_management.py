...
class TestMailResendNoPartner(MailCommon):
    ...
    def setUp(self):
        ...
        self.env['bus.bus'].sudo().search([('channel', 'like', f'%res.partner",{self.user_admin.partner_id.id}%')]).unlink()
            ...
            with self.subTest(name=name):
                ...
                for email_val in [recipient.email, "anotherinvalidaddress", ""]:
                    with self.subTest(recipient_email=email_val):
                        ...
                        with self.assertRaisesRegex(ValidationError, "Email should be re-sent to valid email addresses"):
                            wizard.resend_mail_action()
