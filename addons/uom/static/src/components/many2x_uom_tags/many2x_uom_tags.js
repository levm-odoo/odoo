...
export class Many2XUomTagsAutocomplete extends Many2XAutocomplete {
    ...
    async search(name) {
        ...
        const relativeUomIds = records.filter((record) => record.relative_uom_id).map((record) => record.relative_uom_id[0]);
        const relativeUoms = await this.orm.searchRead(
            "uom.uom",
            [["id", "in", relativeUomIds]], 
            ["id", "name"],
            {
            context: { active_test: false },
            }
        );
        const relativeUomsMap = relativeUoms.reduce((acc, uom) => {
            acc[uom.id] = uom.name;
            return acc;
        }, {});
        return records.map((record) => {
            return {
                ...record,
                relative_info: record.relative_uom_id ? `${record.relative_factor} ${relativeUomsMap[record.relative_uom_id[0]]}` : undefined,
            };
        });
    }
