...
class RepairOrder(models.Model):
    ...
    def _compute_has_uncomplete_moves(self):
        ...
        for repair in self:
            repair.has_uncomplete_moves = any(float_compare(move.quantity, move.product_uom_qty, precision_digits=precision_digits) < 0 for move in repair.move_ids)
    ...
    def action_repair_cancel_draft(self):
        ...
        precision_digits = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        sale_line_to_update = self.move_ids.sale_line_id.filtered(lambda l: l.order_id.state != 'cancel' and float_is_zero(l.product_uom_qty, precision_digits=precision_digits))
