...
class HrExpense(models.Model):
    ...
    nb_attachment = fields.Integer(string="Number of Attachments", compute='_compute_nb_attachment', compute_sudo=True)
    ...
    cannot_approve_reason = fields.Char(string='Cannot Approve Reason', compute='_compute_can_approve', readonly=True)
    ...
    def _compute_is_editable(self):
        ...
        is_approver = self.env.user.has_group('hr_expense.group_hr_expense_team_approver')
        for expense in self:
    ...
    def _compute_state(self):
        ...
        Compute the states of the expense as such (priority is given to the last matching state of the list):
            ...
            - in_payment (or paid): When the move state is 'posted' and it's 'payment_state' is 'in_payment' or 'partial, and there is a residual amount
            - paid: When the move state is posted, and its 'payment_state' is in any other state than 'not_paid' or it is a company paid expense
        """
        for expense in self:
            ...
            if expense.approval_state:
                expense.state = expense.approval_state
                continue
        ...
        for expense in self:
            ...
            if allowed_method_line_ids:
                ...
                expense.selectable_payment_method_line_ids = self.env['account.payment.method.line'].search([
                    ...
                    ('company_id', 'parent_of', expense.company_id.id)
                ])
    ...
    def _compute_has_sister_expenses(self):
        """ Used solely to check if the payment register is available """
        expenses_per_move = dict(self.env['hr.expense']._read_group(
            domain=[('account_move_id', '!=', False)],
            groupby=['account_move_id'],
            aggregates=['id:count'],
        ))
        for expense in self:
            if not expense.account_move_id:
                expense.has_sister_expenses = False
                continue
            expense.has_sister_expenses = bool(expenses_per_move.get(expense.account_move_id.id, 1) - 1)
        ...
        for expense in self:
            if expense.company_id.id not in valid_company_ids:
                ...
                expense.can_reset = True
                    ...
                    (is_team_approver and expense.employee_id.id in child_employee_ids)
                    or expense.employee_id.expense_manager_id == self.env.user
            ):
                expense.can_reset = True
    ...
    def _compute_can_approve(self):
        ...
        for expense in self:
            ...
            is_team_approver |= expense_employee.parent_id.user_id == self.env.user
            if expense.company_id.id not in valid_company_ids:
                ...
                elif self.env.user not in current_managers and not is_approver and expense_employee.expense_manager_id.id != self.env.user.id:
                    reason = _("%(expense_name)s: It is not from your department", expense_name=expense.name)

            expense.can_approve = not reason
            expense.cannot_approve_reason = reason
    ...
    def action_post(self):
        ...
        if company_expenses:
            company_expenses._create_moves()
        ...
        if employee_expenses:
            if len(employee_expenses.company_id) > 1:
                raise UserError(_("You can't post simultaneously expenses belonging to different companies"))
    ...
    def action_pay(self):
        ...
        return self.account_move_id.with_context(default_partner_bank_id=(
            self.account_move_id.partner_bank_id.id if len(self.account_move_id.partner_bank_id.ids) <= 1 else None
        )).action_register_payment()

    def action_reset(self):
        """  """
        self._check_can_reset_approval()
        self.sudo()._reverse_moves()
    ...
    def action_split_wizard(self):
        ...
        if not self.is_editable:
            raise UserError(_("Yo do not have the rights to edit this expense."))
    ...
    def _do_submit(self):
        """ Submit an expense for approbation (without checking access rights) """
        for expense in self:
    ...
    def _post_wizard(self):
        if 'company_account' in set(self.mapped('payment_mode')):
            raise UserError(_("Only expense paid by the employee be posted with the wizard"))
    ...
    def _prepare_bills_vals(self):
        ...
        for employee_sudo, expenses_sudo in self.sudo().grouped('employee_id').items():
            move_ref = expenses_sudo.name if len(expenses_sudo) == 1 else _("Expenses of %(employee)s", employee=employee_sudo.name)
