...
class HrExpenseSheet(models.Model):
    ...
    @api.depends('employee_id', 'employee_id.parent_ids', 'employee_id.expense_manager_id', 'employee_id.department_id')
    def _compute_from_employee_id(self):
        for sheet in self:
            sheet.department_id = sheet.employee_id.department_id
            if sheet.state in ['draft', 'submit'] and not (approver := sheet.employee_id.expense_manager_id):
                ...
                approvers = managers & team_approvers
                if not approvers:
                    approvers = sheet.env.ref('hr_expense.group_hr_expense_user', raise_if_not_found=False).users
                if not approvers:
