    ...
    def _creation_message(self):
        if self.expense_ids:
            if len(self.expense_ids) == 1:
                return _("Expense entry created from this expense: %(link)s", link=self.expense_ids._get_html_link())
            ...
            return _("Expense entry created from the expenses: %(links)s", links=links)
