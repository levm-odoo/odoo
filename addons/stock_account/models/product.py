...
class ProductProduct(models.Model):
    ...
    def _prepare_out_svl_vals(self, quantity, company):
        ...
        if self.product_tmpl_id.cost_method == 'average' and not float_is_zero(self.quantity_svl, precision_rounding=self.uom_id.rounding):
            rounding_error = (self.standard_price * self.quantity_svl - self.value_svl) * abs(quantity / self.quantity_svl)

            # If it is bigger than the (smallest number of the currency * quantity) / 2,
            # then it isn't a rounding error but a stock valuation error, we shouldn't fix it under the hood ...
            if rounding_error and float_compare(rounding_error, max((abs(quantity) * currency.rounding) / 2, currency.rounding), precision_rounding=currency.rounding) < 1:
                vals['value'] += currency.round(rounding_error)
