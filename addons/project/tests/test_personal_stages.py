...
class TestPersonalStages(TestProjectCommon):
    ...
    def test_personal_stage_read_group(self):
        ...
        self.env.flush_all()
        read_group_user = self.env['project.task'].with_context(read_group_expand=True).with_user(self.user_projectuser).web_read_group(
            [('user_ids', '=', self.user_projectuser.id)], aggregates=['sequence:avg', '__count'], groupby=['personal_stage_type_ids'])['groups']
        ...
        read_group_manager = self.env['project.task'].with_context(read_group_expand=True).with_user(self.user_projectmanager).web_read_group(
            [('user_ids', '=', self.user_projectmanager.id)], aggregates=['sequence:avg', '__count'], groupby=['personal_stage_type_ids'])['groups']
