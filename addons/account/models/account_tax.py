...
class AccountTax(models.Model):
    ...
    def _import_base_line_extra_tax_data(self, base_line, extra_tax_data):
        if (
            not extra_tax_data
            or base_line['currency_id'].id != extra_tax_data['currency_id']
            or any(
                not base_line['currency_id'].is_zero(base_line[field_name] - extra_tax_data[field_name])
                for field_name in ('quantity', 'discount', 'price_unit')
            )
            or any(str(tax.id) not in extra_tax_data['manual_tax_amounts'] for tax in base_line['tax_ids'])
        ):
            return {}

        return {
            'computation_key': extra_tax_data['computation_key'],
            'manual_tax_amounts': extra_tax_data['manual_tax_amounts'],
        }
    ...
    def _round_base_lines_tax_details(self, base_lines, company, tax_lines=None):
        ...
        # Round 'total_per_tax'.
        for (_tax, currency, _is_refund, _computation_key), amounts in total_per_tax.items():
    ...
    def _has_taxes_to_exclude(self, base_lines):
        return any(
            base_line['tax_ids'].filtered(lambda tax: not tax._can_be_discounted())
            for base_line in base_lines
        )
    ...
    def _apply_base_lines_manual_amounts_to_reach(self, base_lines, target_amount_currency):
        ...
        if taxes_data:
            ...
            for tax_data in taxes_data:
                tax = tax_data['tax']
                if tax in first_batch:
                    biggest_base_line['manual_tax_amounts'][str(tax.id)]['base_amount_currency'] += delta_reduced_amount_currency
                else:
                    return
            ...
            for base_line in base_lines:
                ...
                for i, tax_data in enumerate(taxes_data):
                    ...
                    if tax._can_be_discounted():
                        new_taxes += tax
                    elif tax.include_base_amount and not tax.price_include:
