...
class AccountMove(models.Model):
    ...
    def create(self, vals_list):
        ...
        with self._check_balanced(container):
            with self.env.protector() as protector, self._sync_dynamic_lines(container):
                for vals in vals_list:
                    self._sanitize_vals(vals)
                stolen_moves = self.browse(set(move for vals in vals_list for move in self._stolen_move(vals)))
                moves = super().create(vals_list)
                protector.protect([protected for vals, move in zip(vals_list, moves) for protected in self._get_protected_vals(vals, move)])
    ...
    def write(self, vals):
        ...
        container = {'records': self | stolen_moves, 'protecting': self._get_protected_vals(vals, self)}
        with self._check_balanced(container):
            with self.env.protector() as protector, self._sync_dynamic_lines(container):
                res = super(AccountMove, self.with_context(
                    skip_account_move_synchronization=True,
                )).write(vals)
                protector.protect(self._get_protected_vals(vals, self))
    ...
    def _disable_recursion(self, container, key, default=None, target=True):
        ...
        stack = self.env.cr.cache.setdefault('disable_recursion_stack', StackMap())
        try:
        ...
        top = stack.pushmap({key: target})
        try:
            yield disabled
        finally:
            assert stack.popmap() is top
