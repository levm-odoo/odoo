...
class PurchaseOrderLine(models.Model):
    ...
    date_planned_pol_placeholder = fields.Char(compute='_compute_date_planned_pol_placeholder', readonly=False, store=True)
    ...
    def _compute_date_planned_pol_placeholder(self):
        for line in self:
            if not line.product_id or line.invoice_lines or not line.company_id:
                continue
            seller = line._get_seller()
            if not line.order_id.origin and (seller or not line.date_planned_pol_placeholder):
                line.date_planned_pol_placeholder = line._get_date_planned(seller).strftime("%Y-%m-%d %H:%M:%S")
            elif line.date_planned and not line.date_planned_pol_placeholder:
                line.date_planned_pol_placeholder = line.date_planned.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def _compute_date_planned(self):
        for line in self:
            ...
            if seller or not line.date_planned:
                if not line.order_id.origin:
                    line.date_planned = datetime.strptime(line.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S").strftime(DEFAULT_SERVER_DATETIME_FORMAT) if line.order_id.state in ('purchase', 'done', 'cancel') else False
                else:
    ...
    def _update_date_planned(self, updated_date):
        ...
        self.date_planned_pol_placeholder = updated_date.strftime("%Y-%m-%d %H:%M:%S")
