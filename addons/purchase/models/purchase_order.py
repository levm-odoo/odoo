...
class PurchaseOrder(models.Model):
    ...
    def _compute_date_planned_placeholder(self):
        for order in self:
            all_dates = [x.date_planned or datetime.strptime(x.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S") for x in order.order_line
    ...
    def onchange_date_planned(self):
        if self.date_planned:
            ...
            self.order_line.filtered(lambda line: not line.display_type).write({'date_planned_pol_placeholder': self.date_planned.strftime("%Y-%m-%d %H:%M:%S"), 'date_planned': self.date_planned})
    ...
    def write(self, vals):
        ...
        if 'partner_id' in vals:
            ...
            for line in self.order_line:
                ...
                line.write({'date_planned': date_planned, 'date_planned_pol_placeholder': date_planned.strftime("%Y-%m-%d %H:%M:%S")})
    ...
    def copy(self, default=None):
        ...
        for line in new_pos.order_line:
            if line.product_id:
                ...
                if not self.origin and not self.date_planned:
                    line.date_planned_pol_placeholder = line._get_date_planned(seller)
                else:
        ...
        if any(self._must_delete_date_planned(field) for field in field_names) and 'value' in result:
            for line in result['value'].get('order_line', []):
                if line[0] == Command.UPDATE:
                    ...
                    if 'date_planned_pol_placeholder' in line[2]:
            ...
            if oldest_rfq:
                ...
                for rfq_line in rfqs.order_line:
                    existing_line = oldest_rfq.order_line.filtered(lambda l: l.product_id == rfq_line.product_id and
                                                                                ...
                                                                                abs(datetime.strptime(l.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S") - datetime.strptime(rfq_line.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S")).total_seconds() <= 86400  # 24 hours in seconds
                                                                        )
    ...
    def _compute_avg_days_to_purchase(self, user_specific=False):
        ...
        query = """
            SELECT AVG(EXTRACT(epoch FROM (po.date_approve - po.create_date)) / (24 * 60 * 60)::decimal(16, 2))
        ...
        return round(result[0] if result and result[0] is not None else 0, 2)
