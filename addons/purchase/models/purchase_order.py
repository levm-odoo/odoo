...
class PurchaseOrder(models.Model):
    ...
    date_planned_placeholder = fields.Char(compute='_compute_date_planned_placeholder', store=True, readonly=False)
    ...
    lock_confirmed_po = fields.Selection(related='company_id.po_lock', string="Purchase Order Modification")
    ...
    def _compute_date_planned_placeholder(self):
        for order in self:
            all_dates = [
                        x.date_planned or datetime.strptime(x.date_planned_pol_placeholder, "%Y-%m-%d %H:%M:%S")
                        for x in order.order_line
                        if not x.display_type and (x.date_planned or x.date_planned_pol_placeholder)
                    ]
            if all_dates:
                order.date_planned_placeholder = min(all_dates).strftime("%Y-%m-%d %H:%M:%S")
            else:
    ...
    def onchange_date_planned(self):
        if self.date_planned:
            self.date_planned_placeholder = self.date_planned.strftime("%Y-%m-%d %H:%M:%S")
            self.order_line.filtered(lambda line: not line.display_type).date_planned_pol_placeholder = self.date_planned.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def onchange(self, values, field_names, fields_spec):
        ...
        if any(self._must_delete_date_planned(field) for field in field_names) and 'value' in result:
            for line in result['value'].get('order_line', []):
                ...
                if line[0] == Command.UPDATE and 'date_planned_pol_placeholder' in line[2]:
                    del line[2]['date_planned_pol_placeholder']
        return result
    ...
    def _compute_avg_days_to_purchase(self, user_specific=False):
        """
        This method calculates the average number of days it takes for purchase orders
        to be approved after their creation within the last three months.
        """
        ...
        total_days_to_purchase = sum((order.date_approve - order.create_date).days for order in total_purchase_orders)
        avg_days_to_purchase = (round(total_days_to_purchase / len(total_purchase_orders), 2) if len(total_purchase_orders) >= 1 else 0)
        return avg_days_to_purchase
