...
class TestSaleProject(HttpCase, TestSaleProjectCommon):
    ...
    def test_group_expand_sales_order(self):
        ...
        context = {'gantt_start_date': '2023-01-01', 'gantt_scale': 'month'}
        domain = [
            ('planned_date_begin', '>=', Datetime.to_datetime('2023-01-01')),
            ('date_deadline', '<=', Datetime.to_datetime('2023-01-04')),
        ]

        displayed_sale_order = self.env['project.task'].with_context(context)._group_expand_sales_order(None, domain)
        self.assertFalse(
            displayed_sale_order,
            'Sale orders without scheduled tasks should not be displayed in the Gantt view'
        )

        displayed_sale_order = self.env['project.task'].with_context(context)._group_expand_sales_order(
            None,
            [('sale_order_id', 'ilike', 'Test')] + domain,
        )
        self.assertEqual(
            order,
            displayed_sale_order,
            'The matching sale order should be displayed in the Gantt view'
        )
