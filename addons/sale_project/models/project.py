...
class ProjectTask(models.Model):
    ...
    def _compute_sale_line(self):
        for task in self:
            ...
            if self._context.get('skip_recursion_compute'):
                continue

            sale_line = False
            context_no_recursion = dict(self._context, skip_recursion_compute=True)

            # Determine the sale_line_id
            if task.parent_id.sale_line_id and task.parent_id.partner_id.commercial_partner_id == task.partner_id.commercial_partner_id:
                sale_line = task.with_context(context_no_recursion).parent_id.sale_line_id
                ...
                sale_line = task.with_context(context_no_recursion).project_id.sale_line_id

            # If no valid sale_line is found, fall back to milestone's sale_line_id
            task.sale_line_id = sale_line or task.milestone_id.sale_line_id

            # Validate the computed sale_line_id against the customer
            if task.sale_line_id and task.sale_line_id.order_partner_id.commercial_partner_id != task.partner_id.commercial_partner_id:
                task.sale_line_id = False
