...
class L10nEsEdiTbaiDocument(models.Model):
    ...
    def _check_can_post(self, values):
        ...
        if not values['is_sale'] :
            if self.company_id.vat and len(self.company_id.vat) > 1 and not self.company_id.vat[-1].isdecimal():
    ...
    def _prepare_post_params_bi(self, is_sale):
        ...
        company = self.company_id
        freelancer = len(company.vat) > 1 and not company.vat[-1].isdecimal()
