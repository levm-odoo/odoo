...
const searchPanelDomainImage = (model, fieldName, domain, setCount = false, limit = false) => {
    ...
    if (isM2OField(field)) {
        ...
        // web_formatted_read_group does not take care of the condition [fieldName, '!=', false]
...
export class Model extends Array {
     ...
     * @param {string} [order]
     */
    web_formatted_read_group(domain, groupby, aggregates, having, offset, limit, order) {
    ...
    read_progress_bar(domain, groupBy, progressBar) {
        ...
        const groups = this.web_formatted_read_group(domain, [groupBy, progressBar.field], ['__count']);
    ...
    web_read_group(domain, groupby, aggregates, limit, offset, order) {
        ...
        const groups = this.web_formatted_read_group(kwargs);
        const allGroups = this.web_formatted_read_group(domain, groupby, []);
