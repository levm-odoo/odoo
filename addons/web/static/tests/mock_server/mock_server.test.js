...
test("performRPC: web_read_group, no group", async function (assert) {
    await makeMockServer();
    const result = await ormRequest({
        model: "bar",
    ...
    expect(result.groups).toEqual([{ __count: 0, __extra_domain: [] }]);
});

test("performRPC: web_read_group, group by char", async () => {
    await makeMockServer();
    const result = await ormRequest({
        model: "bar",
        method: "web_read_group",
...
test("performRPC: web_read_group, group by boolean", async () => {
    await makeMockServer();
    const result = await ormRequest({
        model: "bar",
        method: "web_read_group",
...
test("performRPC: web_read_group, group by date", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group, group by date with number granularity", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group, group by datetime", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group, group by datetime with number granularity", async () => {
    await makeMockServer();
    ...
    for (const { granularity, result, count } of allGranularity) {
        ...
        expect(response.groups.map((x) => x[`datetime:${granularity}`])).toEqual(result);
        expect(response.groups.map((x) => x.__count)).toEqual(count);
        expect(response.groups.map((x) => x.__extra_domain)).toEqual(
            result.map((r) => [[`datetime.${granularity}`, "=", r]])
        );
    }
});

test("performRPC: web_read_group, group by m2m", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group, order by date with granularity", async () => {
    await makeMockServer();
    let result = (await ormRequest({
        model: "bar",
        method: "web_read_group",
...
test("performRPC: web_read_group, group by m2o", async () => {
...
test("performRPC: web_read_group, group by integer", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group, group by selection", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group, group by two levels", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group with special measure specifications", async () => {
...
test("performRPC: web_read_group with array_agg", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group with array_agg on id", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group with array_agg on an integer field", async () => {
    await makeMockServer();
...
test("performRPC: web_read_group with count_distinct", async () => {
    await makeMockServer();
