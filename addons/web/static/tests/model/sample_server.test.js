...
describe("RPC calls", () => {
    ...
    test("'web_read_group': groupBy", async () => {
        ...
        expect(result.groups).toHaveLength(3);
        expect(result.groups.map((g) => g.profession)).toEqual(["adventurer", "brewer", "gardener"]);
    ...
    test("'web_read_group': groupBy and field", async () => {
        ...
        expect(result.groups.map((g) => g.profession)).toEqual(["adventurer", "brewer", "gardener"]);
        expect(result.groups.reduce((acc, g) => acc + g.__count, 0)).toBe(MAIN_RECORDSET_SIZE);
        expect(result.groups.reduce((acc, g) => acc + g['age:sum'], 0)).toBe(
            server.data.hobbit.records.reduce((acc, rec) => acc + rec.age, 0)
        );
    });

    test("'web_read_group': multiple groupBys", async () => {
        const server = new DeterministicSampleServer("hobbit", fields.hobbit);
        const result = await server.mockRpc({
            method: "web_read_group",
            model: "hobbit",
        ...
        expect("profession" in result.groups[0]).toBe(true);
        expect("age" in result.groups[0]).toBe(true);
    });
    ...
    test("'web_read_group': multiple groupBys among which a many2many", async () => {
        ...
        expect(result.groups[0].tag_ids[0]).toBeOfType("number");
        expect(result.groups[0].tag_ids[1]).toBeOfType("string");
    });
    ...
    test("'web_read_group': partial support of array_agg", async () => {
        ...
        expect(result.groups[0]["id:array_agg"]).toEqual(ids);
    });
});
