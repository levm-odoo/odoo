...
export class MockServer {
    ...
    mockReadGroup(modelName, kwargs) {
        ...
        function formatValue(groupByField, val) {
            ...
            if (['date', 'datetime'].includes(type)) {
                ...
                const raw_value = type === 'date'? serializeDate(start_date) : serializeDateTime(start_date)
        ...
        for (const [groupKey, groupRecords] of Object.entries(groups)) {
            ...
            for (const groupbySpec of groupby) {
                ...
                let value = Number.isInteger(group[groupbySpec])
                    ? group[groupbySpec]
                    : group[groupbySpec] || false;
                const { relation, type } = fields[fieldName];

                if (relation && !Array.isArray(value)) {
                    const relatedRecord = this.models[relation].records.find(({ id }) => id === value);
                    if (relatedRecord) {
                ...
                if (['date', 'datetime'].includes(type)) {
                    if (value) {
                        if (!READ_GROUP_NUMBER_GRANULARITY.includes(granularity)) {
                            ...
                            startDate = type === 'date' ? deserializeDate(value[0]) : deserializeDateTime(value[0]);
                            let label = value[1];
                            switch (granularity) {
                                case "hour": {
    ...
    mockReadProgressBar(modelName, kwargs) {
        ...
        const groups = this.mockReadGroup(
