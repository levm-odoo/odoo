...
test.tags("desktop");
test("Scroll right Kanban when unfold", async () => {
    for (let i = 0; i < 12; i++) {
        Product._records.push({ id: 8 + i, name: `column ${i}` });
        Partner._records.push({ id: 20 + i, foo: "dumb entry", product_id: 8 + i });
    }
    ...
    const foldscroll = content.scrollLeft;
    await contains(".o_kanban_renderer div:nth-child(9)").click();
    const unfoldscroll = content.scrollLeft;
    expect(unfoldscroll).toBe(foldscroll + 300);
});
