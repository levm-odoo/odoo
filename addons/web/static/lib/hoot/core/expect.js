...
export class CaseResult {
     ...
     */
    getEvents(type) {
        const nType = CASE_EVENT_TYPES[type];
    ...
    registerEvent(type, value) {
        ...
        this.events.push(markRaw(caseEvent));
        this.counts[type] ||= 0;
        this.counts[type]++;
    }
}
...
export class Assertion extends CaseEvent {
    type = CASE_EVENT_TYPES.assertion;
        ...
        for (const part of parts) {
            if (part instanceof ElementMap) {
                if (typeof part.selector === "string") {
                    ...
                    this.message.push(
                        "elements",
                        makeLabelOrString(elements.length === 1 ? elements[0] : elements)
                    );
                }
...
export class DOMCaseEvent extends CaseEvent {
    ...
    constructor(type, [name, args, returnValue]) {
        ...
        this.type = CASE_EVENT_TYPES[type];
        this.label = name;
