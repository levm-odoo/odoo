...
// Defined with string concat because line returns would be taken into account in
// <pre> tags.
const FORMATTED_STACK_TEMPLATE =
    /* xml */ `<pre class="hoot-technical m-0">` +
    /* xml */ `<t t-foreach="splitStack(error.stack)" t-as="part" t-key="part_index">` +
...
const ERROR_TEMPLATE = /* xml */ `
    ...
    <div class="flex col-span-2 gap-2 px-2">
        <span class="text-fail">Source:</span>
        <t t-set="stack" t-value="error.stack" />
        ${FORMATTED_STACK_TEMPLATE}
    </div>
    <t t-if="error.cause">
        <div class="flex col-span-2 gap-2 px-2">
            <span class="text-fail">Cause:</span>
            <t t-set="stack" t-value="error.cause.stack" />
            ${FORMATTED_STACK_TEMPLATE}
        </div>
    </t>
`;
...
const STEP_TEMPLATE = /* xml */ `
    <div class="text-muted flex items-center gap-1 px-2 truncate">
        ...
        <span class="flex gap-1 truncate items-center">
            <t t-if="step.message.type and step.message.type !== 'raw'">
