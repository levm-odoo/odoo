...
class Base(models.AbstractModel):
    ...
    def web_read_group(self, domain, groupby=(), aggregates=(), having=(), offset=0, limit=None, order=None):
        """
        Returns the result of a web_formatted_read_group and the total number of groups matching the search domain.

        :param domain: search domain
        :param groupby: list of groupby specification (see ``groupby``` param of ``web_formatted_read_group``)
        :param aggregates: list of aggregate specification (see ``aggregates``` param of ``web_formatted_read_group``)
        :param limit: see ``limit`` param of ``web_formatted_read_group``
        :param offset: see ``offset`` param of ``web_formatted_read_group``
        :param order: forced order, see ``order`` param of ``web_formatted_read_group``
        :return: {
            'groups': array of read groups
            'length': total number of groups
        }
        """
        groups = self.web_formatted_read_group(domain, groupby, aggregates, having, offset, limit, order)
    ...
    @api.model
    @api.readonly
    def web_formatted_read_group(self, domain, groupby=(), aggregates=(), having=(), offset=0, limit=None, order=None) -> list[dict]:
        ...
        if (
            self.env.context.get('read_group_expand') and
            len(groupby) == 1 and
            not offset and
            (not limit or len(groups) < limit)
        ):
            # TODO: doesn't respect the order/having
    ...
    def _web_read_group_format(self, groupby, aggregates, groups) -> list[dict]:
        """ Format raw value of _read_group for the webclient.
        See web_formatted_read_group return value. """
        ...
        for groupby_spec, values in zip(groupby, column_iterator):
            ...
            if not (
                ...
                field.relational and field.group_expand
            ):
                continue
        ...
        for aggregate_spec, values in zip(aggregates, column_iterator):
            for value, dict_group in zip(values, result, strict=True):
    ...
    def _search_panel_domain_image(self, field_name, domain, set_count=False, limit=False):
        ...
        groups = self.web_formatted_read_group(domain, [field_name], ['__count'], limit=limit)
