...
class HrLeave(models.Model):
    ...
    def _check_date(self):
        ...
        for holiday in self:
            if holiday.dashboard_warning_message:
                raise UserError(holiday.dashboard_warning_message)
    ...
    @api.depends(
        ...
        'leave_type_request_unit', 'number_of_days',
    )
    ...
    def action_validate(self, check_state=True):
        ...
        if check_state and any(not holiday.can_validate for holiday in self):
            raise UserError(_('You can\'t validate this record.'))
        if leaves:
    ...
    def _check_approval_update(self, state):
        ...
        for holiday in self:
            ...
            if is_in_past and not is_time_off_administrator:
                raise UserError('Only a Time Off Manager can change the state of a leave in the past.')
