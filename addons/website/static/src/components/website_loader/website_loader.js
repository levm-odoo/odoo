...
export class WebsiteLoader extends Component {
    ...
    setup() {
        ...
        useBus(this.props.bus, "SHOW-WEBSITE-LOADER", (ev) => {
            ...
            if (props?.loadingMessages?.length) {
                ...
                this.currentWaitingMessage = false;
                this.waitingMessages = [];
            }
        });
        useBus(this.props.bus, "HIDE-WEBSITE-LOADER", async () => {
    ...
    updateWaitingMessage() {
        ...
        if (this.currentWaitingMessageIndex !== progressBasedMessageIndex) {
            ...
            for (let i = this.currentWaitingMessageIndex; i < progressBasedMessageIndex; i++) {
                if (!this.waitingMessages[i].completed) {
    ...
    async gracefulClose() {
        ...
        if (remainingMessagesLength === 0) {
            ...
            // Quickly show all the remaining messages and complete the progress
            // bar
            for (let i = this.currentWaitingMessageIndex; i < this.waitingMessages.length; i++) {
        ...
        // Wait for a moment to ensure the user sees that all the messages are
        // completed
