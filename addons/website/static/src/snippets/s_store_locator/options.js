...
options.registry.StoreLocator = options.Class.extend({
    ...
    onBuilt() {
        ...
        // again.
        if (!this.$target[0].querySelector('.s_store_locator_embedded')) {
            const iframeEl = generateGMapIframe();
            this.$target[0].querySelector('.s_store_locator_color_filter').before(iframeEl);
            this._updateSource();
        }
    },
    ...
    _updateSource() {
        ...
        const $embedded = this.$target.find('.s_store_locator_embedded');
