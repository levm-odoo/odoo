...
export class PaymentPinelabs extends PaymentInterface {
    ...
    async handlePinelabsPaymentResponse(response, paymentLine) {
        ...
        let pinelabsLatestResponse = await this.pos.data.silentCall(
            "pos.payment.method",
            "get_pinelabs_latest_reponse",
            [[this.payment_method_id.id]]
        );
        pinelabsLatestResponse = JSON.parse(pinelabsLatestResponse);
        if (response.error) {
            return this._pinelabsHandleResponse(response);
        }
        let isPaymentSuccessful = false;
        if (
            pinelabsLatestResponse.responseCode === 0 &&
            pinelabsLatestResponse.plutusTransactionReferenceID !==
                parseInt(paymentLine.plutus_transaction_reference)
        ) {
            return this._pinelabsHandleResponse({ error: _t("Reference number mismatched") });
        } else if (pinelabsLatestResponse.responseCode === 0) {
            const data = pinelabsLatestResponse.data;
