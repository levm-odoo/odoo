...
export class PaymentPinelabs extends PaymentInterface {
    ...
    async _pinelabsCancel() {
        ...
        // In this scenario, we set a timeout for this.paymentInitialTime + CANCEL_REQUEST_TIME_LIMIT_MS - Date.now() milliseconds for the second request.
        ...
        if (response.responseCode === 11) {
            this.pos.notification.add(
                _t(
                    "Your cancellation request with Plutus Transaction Reference ID %s is in the queue and will be processed shortly.",
                    data.plutusTransactionReferenceID
                )
            );
            setTimeout(async () => {
                const response = await this._callPinelabs(data, "pinelabs_cancel_payment_request");
            ...
            }, this.paymentInitialTime + CANCEL_REQUEST_TIME_LIMIT_MS - Date.now());
            return true;
        }
