...
class TestLoyalty(TestSaleCouponCommon):
    ...
    def test_reward_cheapest_product_applied_once_when_clear_wallet(self):
        ...
        msg = (
            f"Total amount should be 1122.62 (previous total - 15*1.15*0.5 discounted) and not"
            f" {order.amount_total}."
        )
        self.assertEqual(float_compare(order.amount_total, 1122.62, precision_rounding=3), 0, msg)

    def test_reward_specific_product_with_prorata_when_cheapest_already_applied(self):
        """ Check the application of a specific reward is computed with a prorata on what isn't yet
         discounted. """
