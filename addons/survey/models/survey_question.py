...
class SurveyQuestion(models.Model):
    ...
    def _validate_date(self, answer):
        ...
        try:
            dateanswer = fields.Datetime.from_string.to_datetime(answer) if isDatetime else fields.Date.from_string.to_datetime(answer)
        except ValueError:
            return {self.id: _('This is not a date')}
        if self.validation_required:
            # Check if answer is in the right range
            if isDatetime:
                min_date = fields.Datetime.from_string.to_datetime(self.validation_min_datetime)
                max_date = fields.Datetime.from_string.to_datetime(self.validation_max_datetime)
                dateanswer = fields.Datetime.from_string.to_datetime(answer)
            else:
                min_date = fields.Date.from_string.to_datetime(self.validation_min_date)
                max_date = fields.Date.from_string.to_datetime(self.validation_max_date)
                dateanswer = fields.Date.from_string.to_datetime(answer)
