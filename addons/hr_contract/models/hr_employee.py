    ...
    def _get_unusual_days(self, date_from, date_to=None):
        ...
        for contract in employee_contracts:
            ...
            unusual_days.update(contract.resource_calendar_id.sudo(False)._get_unusual_days(
                datetime.combine(fields.Date.from_string.to_datetime(tmp_date_from), time.min).replace(tzinfo=UTC),
                datetime.combine(fields.Date.from_string.to_datetime(tmp_date_to), time.max).replace(tzinfo=UTC),
                self.company_id,
            ))
        return unusual_days
