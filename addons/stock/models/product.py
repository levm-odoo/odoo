...
class ProductProduct(models.Model):
    ...
    qty_available = fields.Float(
        ...
        digits='Product Unit of Measure', compute_sudo=False, inverse='_inverse_qty_available',
    ...
    def _inverse_qty_available(self):
        ...
        for product in self:
            if (
                product.type == "consu" and product.is_storable and
                (product.qty_available > 0 or product.product_tmpl_id.qty_available > 0)
            ):
                ...
                self.env['stock.quant'].with_context(inventory_mode=True).create({
                    ...
                    'inventory_quantity': product.qty_available or product.product_tmpl_id.qty_available,
                })._apply_inventory()
...
class ProductTemplate(models.Model):
    ...
    qty_available = fields.Float(
        'Quantity On Hand', compute='_compute_quantities', search='_search_qty_available',
        inverse='_inverse_qty_available', compute_sudo=False, digits='Product Unit of Measure')
    virtual_available = fields.Float(
        'Forecasted Quantity', compute='_compute_quantities', search='_search_virtual_available',
    ...
    def _compute_show_qty_update_button(self):
        for product in self:
            product.show_qty_update_button = False
            if product._should_open_product_quants() or product.product_variant_count > 1:
    ...
    def _inverse_qty_available(self):
        ...
        for template in self:
            ...
            template.product_variant_id._inverse_qty_available()
    ...
    def action_update_quantity_on_hand(self):
        self.ensure_one()
        if self._should_open_product_quants():
