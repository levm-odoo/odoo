...
class StockWarehouseOrderpoint(models.Model):
    ...
    replenishment_uom_id = fields.Many2one(
        'uom.uom', 'Replenishment Unit', help="The procurement quantity will be rounded up to a multiple of this unit/packagins. If it is not set, it is not rounded.")
    ...
    def _compute_unwanted_replenish(self):
        ...
        for orderpoint in self:
            if not orderpoint.product_id or float_is_zero(orderpoint.qty_to_order, precision_digits=precision_digits) or float_compare(orderpoint.product_max_qty, 0, precision_digits=precision_digits) == -1:
                ...
                orderpoint.unwanted_replenish = float_compare(after_replenish_qty, orderpoint.product_max_qty, precision_digits=precision_digits) > 0
    ...
    def _onchange_route_id(self):
        if self.route_id:
            self.replenishment_uom_id = self._get_replenishment_uom_to_order()
    ...
    def action_replenish(self, force_to_max=False):
        ...
        if force_to_max:
            for orderpoint in self:
                ...
                if not float_is_zero(remainder, precision_digits=self.env['decimal.precision'].precision_get('Product Unit of Measure')):
    ...
    def _get_qty_to_order(self, force_visibility_days=False, qty_in_progress_by_orderpoint={}):
        ...
        if float_compare(self.qty_forecast, self.product_min_qty, precision_digits=rounding) < 0:
    ...
    def _get_replenishment_uom_to_order(self):
    ...
    def _get_orderpoint_action(self):
        ...
        for loc in all_replenish_location_ids:
            for product in all_product_ids:
                ...
                if float_compare(qty_available + incoming_qty - outgoing_qty, 0, precision_digits=rounding) < 0:
        ...
        for (days, loc), product_ids in ploc_per_day.items():
            ...
            for (product, qty) in zip(products, qties):
                if float_compare(qty['virtual_available'], 0, precision_digits=rounding) < 0:
        ...
        for orderpoint in orderpoints:
            ...
            orderpoint.replenishment_uom_id = orderpoint._get_replenishment_uom_to_order()
        return action
            ...
            try:
                ...
                while orderpoints_batch:
                    ...
                    for orderpoint in orderpoints_batch:
                        ...
                        if float_compare(orderpoint.qty_to_order, 0.0, precision_digits=self.env['decimal.precision'].precision_get('Product Unit of Measure')) == 1:
