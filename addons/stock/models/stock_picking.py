...
class StockPicking(models.Model):
    ...
    def _compute_products_availability(self):
        ...
        for picking in pickings:
            ...
            if any(float_compare(move.forecast_availability, 0 if move.state == 'draft' else move.product_qty, precision_digits=precision_digits) == -1 for move in picking.move_ids):
    ...
    def _get_lot_move_lines_for_sanity_check(self, none_done_picking_ids, separate_pickings=True):
        ...
        def get_line_with_done_qty_ids(move_lines):
            ...
            precision_digits = self.env['decimal.precision'].precision_get('Product Unit of Measure')
            return move_lines.filtered(lambda ml: ml.product_id and ml.product_id.tracking != 'none' and ml.picked and float_compare(ml.quantity, 0, precision_digits=precision_digits)).ids
