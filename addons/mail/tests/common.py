...
class MockEmail(common.BaseCase, MockSmtplibCase):
    ...
    def _find_mail_mail_wemails(self, email_to_all, status, mail_message=None, author=None, content=None, email_from=None):
        ...
        if isinstance(email_to_all, str):
            email_to_all = email_to_all.split(',')
        email_to = set(email_normalize(email) or email for email in email_to_all or []) - {''}

        for mail in filtered:
            mail_email_addrs = mail.email_to and set(email_normalize(email) or email for email in mail.email_to.split(',')) or set()
            mail_partner_email_addrs = set(email_normalize(email) or email for email in mail.recipient_ids.mapped('email'))
        ...
        for message_info in recipients_info:
            ...
            if messages:
                message = messages.filtered(lambda message: (
                    ...
                    (msubtype is None or message.subtype_id == msubtype)
                ))
            else:
            ...
            for recipient in message_info['notif']:
                ...
                notif = notifications.filtered(
                    ...
                    ((partner and n.res_partner_id == partner) or n.email in email_to_lst) and
