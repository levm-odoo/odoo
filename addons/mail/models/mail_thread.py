...
class MailThread(models.AbstractModel):
    ...
    def _notify_thread_by_email(self, message, recipients_data, msg_vals=False,
        ...
        for _lang, render_values, recipients_group in self._notify_get_classified_recipients_iterator(
            ...
            recipients = [(pid, 'pid') for pid in recipients_group.get('recipients_ids', [])]
            recipients += [(email, 'email') for email in recipients_group.get('recipients_emails', [])]

            # create email
            for recipients_chunk in split_every(gen_batch_size, recipients):
                ...
                if new_email and recipients_chunk:
                    tocreate_recipient_ids = [pid for pid, r_type in recipients_chunk if r_type == 'pid']
                    tocreate_emails = [email for email, _ in recipients_chunk[len(tocreate_recipient_ids):]]
                    if resend_existing:
                        ...
                        if existing_notifications:
                            tocreate_recipient_ids = [rid for rid in tocreate_recipient_ids if rid not in existing_notifications.mapped('res_partner_id.id')]
    ...
    def _notify_by_email_get_final_mail_values(self, recipients, mail_values,
        ...
        :param list[tuple] recipients: res.partner IDs to notify or emails as [(1, 'pid'), ('test@test.lan', 'email')];
        ...
        final_mail_values['recipient_ids'] = [Command.link(pid) for pid, r_type in recipients if r_type == 'pid']
        final_mail_values['email_to'] = ','.join(email for email, r_type in recipients if r_type == 'email')
        if additional_values:
