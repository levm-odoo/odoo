...
class MailNotification(models.Model):
    ...
    _notification_partner_or_email_required = models.Constraint(
        "CHECK(notification_type != 'email' OR failure_type IS NOT NULL OR res_partner_id IS NOT NULL OR COALESCE(email, '') != '')",
    ...
    def create(self, vals_list):
        # setting this when the compute would not set it anyway
        # prevents a pointless fetch for the compute
        for v in vals_list:
            if v.get('notification_type', 'inbox') != 'email':
                v['email'] = False
