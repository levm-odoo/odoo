    ...
    def resend_mail_action(self):
        ...
        for wizard in self:
            ...
            if any(not r.partner_id and not email_normalize(r.email) for r in to_send):
                raise ValidationError(_("Email should be re-sent to valid email addresses."))
    ...
    def _inverse_details(self):
        ...
        for recipient in self - partner_recipients:
            if not recipient.notification_id.failure_type:
                raise ValidationError(_("You may not edit the email for a notification that is not in a failed state."))
