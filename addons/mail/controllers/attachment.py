...
class AttachmentController(http.Controller):
    ...
    def mail_attachment_upload(self, ufile, thread_id, thread_model, is_pending=False, **kwargs):
        ...
        allowed_params = self._whitelist_upload_kwargs(request.env[thread_model])
        if invalid := (set(kwargs.keys()) - allowed_params):
            raise ValueError(f"Invalid parameters to mail_attachment_upload {invalid }")
    ...
    def _whitelist_upload_kwargs(self, thread):
        return thread._get_allowed_access_params() | {
            'activity_id',  # when uploading for an activity, used notably in documents
            'cloud_storage',  # used in cloud_storage sub modules
            'tmp_url', 'temporary_id',  # generated by JS, should probably not land in final route
        }
    ...
    def mail_attachment_delete(self, attachment_id, access_token=None, **kwargs):
        ...
        # sanity check on kwargs
        allowed_params = self._whitelist_delete_kwargs(request.env[attachment_message.model or 'mail.thread'])
        if invalid := (set(kwargs.keys()) - allowed_params):
            raise ValueError(f"Invalid parameters to mail_attachment_delete {invalid }")
    ...
    def _whitelist_delete_kwargs(self, thread):
        return thread._get_allowed_access_params()

    @http.route(['/mail/attachment/zip'], methods=["POST"], type="http", auth="public")
    def mail_attachment_get_zip(self, file_ids, zip_name):
