...
class StockMove(models.Model):
    ...
    def _get_price_unit(self):
        ...
        uom_prec = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        line = self.purchase_line_id
        order = line.order_id
        received_qty = line.qty_received
        if self.state == 'done':
            received_qty -= self.product_uom._compute_quantity(self.quantity, line.product_uom_id, rounding_method='HALF-UP')
        if line.product_id.purchase_method == 'purchase' and float_compare(line.qty_invoiced, received_qty, precision_digits=uom_prec) > 0:
    ...
    def _account_entry_move(self, qty, description, svl_id, cost):
        ...
        if not am_vals_list or not self.purchase_line_id or pdiff_exists or float_is_zero(qty, precision_digits=precision_digits):
            return am_vals_list
