...
const CLIPBOARD_BLACKLISTS = {
    // Use with caution:
    // this list does not account for complex baseContainers using a combination of
    // tag + class/attributes
    unwrap: [
        // These elements' children will be unwrapped.
        ".Apple-interchange-newline",
        "DIV", // DIV is unwrapped unless eligible for a baseContainer, see cleanForPaste
    ],
...
export class ClipboardPlugin extends Plugin {
        ...
        for (const textFragment of textFragments) {
            ...
            if (textIndex < textFragments.length) {
                ...
                if (
                    ...
                    const divFactory = this.dependencies.baseContainer.getFactory("DIV");
                    if (
                        block &&
                        block.matches(this.dependencies.baseContainer.getGlobalSelector()) &&
                        blockBefore &&
                        !blockBefore.matches(divFactory.selector)
                    ) {
                        // Do something only if blockBefore is not a DIV (which is the no-margin option)
                        // replace blockBefore by a DIV.
                        const div = divFactory.create();
    ...
    prepareClipboardData(clipboardData) {
        ...
        const closestBaseContainer =
            ...
            closestElement(
                selection.anchorNode,
                this.dependencies.baseContainer.getGlobalSelector()
            );
        ...
        for (const br of brs) {
            ...
            if (
                ...
                // TODO baseContainer: I don't know why but we shouldn't do that with pre (see tests)
                block.nodeName !== "PRE" &&
                !block.closest("li")
            ) {
        ...
        if (
            ...
            if (!node.matches || node.matches(CLIPBOARD_BLACKLISTS.remove.join(","))) {
                ...
                if (node.nodeName === "DIV") {
                    if (this.dependencies.baseContainer.isEligibleForBaseContainer(node)) {
