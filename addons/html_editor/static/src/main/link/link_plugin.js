...
export class LinkPlugin extends Plugin {
    ...
    normalizeLink(root) {
        ...
        for (const anchorNode of selectElements(root, "a")) {
            const { color } = anchorNode.style;
            ...
            if (color && [...anchorNode.childNodes].every((n) => !isBlock(n))) {
                anchorNode.style.removeProperty("color");
                for (const font of selectElements(anchorNode, "font")) {
                    if (anchorNode.textContent === font.textContent) {
                        return;
                    }
                }
                const font = document.createElement("font");
                font.append(...anchorNode.childNodes);
                anchorNode.appendChild(font);
                this.dependencies.color.colorElement(font, color, "color");
            }
        }
    }
