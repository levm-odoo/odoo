...
class MrpWorkcenter(models.Model):
    ...
    # def _get_capacity(self, product, unit):
    #     capacity = self.capacity_ids.filtered(lambda capacity: capacity.product_id == product and capacity.product_uom_id == unit)
    #     if not capacity:
    ...
    # def _get_expected_duration(self, product, unit):
    ...
    #     """
    #     # capacity = self.capacity_ids.filtered(lambda p: p.product_id == product_id and capacity.product_uom_id == unit)
    #     # return capacity.time_start + capacity.time_stop if capacity else self.time_start + self.time_stop
    #     capacity = self.capacity_ids.filtered(lambda capacity: capacity.product_id == product and capacity.product_uom_id == unit)
    #     if not capacity:
    #         capacity = self.capacity_ids.filtered(lambda capacity: not capacity.product_id and capacity.product_uom_id == unit)
    #     if capacity:
    #         return capacity.time_start + capacity.time_stop
    #     return self.time_start + self.time_stop

    def _find_capacity(self, product, unit, default_capacity=1):
        capacity = self.capacity_ids.filtered(lambda capacity: capacity.product_id == product and capacity.product_uom_id == unit)
        if not capacity:
            capacity = self.capacity_ids.filtered(lambda capacity: not capacity.product_id and capacity.product_uom_id == unit)
        if capacity:
            return (capacity.capacity, capacity.time_start, capacity.time_stop)
...
class MrpWorkcenterCapacity(models.Model):
    ...
    # _unique_workcenter_product_product_uom = models.Constraint(
    #     # 'UNIQUE NULLS NOT DISTINCT (workcenter_id, product_id, product_uom_id)',
    #     'UNIQUE(workcenter_id, COALESCE(product_id, 0), product_uom_id)',
    #     'Product capacity should be unique for each workcenter.',
    # )
    _workcenter_product_product_uom_unique = models.UniqueIndex('(workcenter_id, COALESCE(product_id, 0), product_uom_id)')
    # pr upgrade...
