...
class MrpWorkorder(models.Model):
    ...
    product_uom_id = fields.Many2one('uom.uom', 'Unit', required=True, readonly=True)
    ...
    def _compute_state(self):
        ...
        for workorder in self:
            ...
            has_qty_ready = float_compare(workorder.qty_ready, 0, precision_digits=precision_digits) > 0
    ...
    def write(self, values):
        ...
        if 'qty_produced' in values:
            ...
            elif float_compare(values['qty_produced'], 0, precision_digits=self.env['decimal.precision'].precision_get('Product Unit of Measure')) < 0:
    ...
    def _compute_qty_remaining(self):
        ...
        for wo in self:
            if wo.production_id.product_uom_id:
                wo.qty_remaining = max(float_round(wo.qty_production - wo.qty_reported_from_previous_wo - wo.qty_produced, precision_digits=precision_digits), 0)
            else:
                wo.qty_remaining = 0
