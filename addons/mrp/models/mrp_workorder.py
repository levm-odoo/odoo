...
class MrpWorkorder(models.Model):
    ...
    def _get_duration_expected(self, alternative_workcenter=False, ratio=1):
        ...
        capacity, setup, cleanup = self.workcenter_id._find_capacity(self.product_id, self.product_id.uom_id, self.production_bom_id.product_qty or 1)
        if not self.operation_id:
            ...
            if self.qty_producing not in (0, self.qty_production, self._origin.qty_producing):
                ...
                qty_ratio = 1
            # return self.workcenter_id._get_expected_duration(self.product_id, self.product_uom_id) + duration_expected_working * qty_ratio * ratio * 100.0 / self.workcenter_id.time_efficiency
        ...
        # capacity = self.workcenter_id._get_capacity(self.product_id, self.product_uom_id)
        ...
        if alternative_workcenter:
            ...
            # duration_expected_working = (self.duration_expected - self.workcenter_id._get_expected_duration(self.product_id, self.product_uom_id)) * self.workcenter_id.time_efficiency / (100.0 * cycle_number)
            duration_expected_working = (self.duration_expected - setup - cleanup) * self.workcenter_id.time_efficiency / (100.0 * cycle_number)
            if duration_expected_working < 0:
                duration_expected_working = 0
            # capacity = alternative_workcenter._get_capacity(self.product_id, self.product_uom_id)
            capacity, setup, cleanup = alternative_workcenter._find_capacity(self.product_id, self.product_id.uom_id, self.production_bom_id.product_qty or 1)
            # alternative_wc_cycle_nb = float_round(qty_production / capacity, precision_digits=0, rounding_method='UP')
            cycle_number = float_round(qty_production / capacity, precision_digits=0, rounding_method='UP')
            # return alternative_workcenter._get_expected_duration(self.product_id, self.product_uom_id) + alternative_wc_cycle_nb * duration_expected_working * 100.0 / alternative_workcenter.time_efficiency
            return setup + cleanup + cycle_number * duration_expected_working * 100.0 / alternative_workcenter.time_efficiency
        time_cycle = self.operation_id.time_cycle
        # return self.workcenter_id._get_expected_duration(self.product_id, self.product_uom_id) + cycle_number * time_cycle * 100.0 / self.workcenter_id.time_efficiency
